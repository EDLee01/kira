...
import { MagiPaths, getRuntimeSettings } from "./paths.ts";
...
export interface MagiConfig {
  ...
  models: {
    ...
    router?: Record<string, import("./routing/model-router.ts").ModelCapabilities>;
  };
  mcp: {
...
function readRouterConfig(
  value: Record<string, unknown>,
  configFile: string
): Record<string, import("./routing/model-router.ts").ModelCapabilities> | undefined {
  if (Object.keys(value).length === 0) {
    return undefined;
  }
  const result: Record<string, import("./routing/model-router.ts").ModelCapabilities> = {};
