...
import { ensureMemoryStructure, listMemoryFiles, memoryRoot, MemoryRootOptions } from "./memory-files.ts";
import { MemoryDraft, proposeMemoryDraft } from "./memory-draft.ts";
import { recordMemoryAudit } from "./memory-audit.ts";
...
function analyzeMemory(input: MemoryRootOptions): DreamOperation[] {
  ...
  operations.push(...findPolicyConflicts(files));
  return operations.slice(0, 20);
}

function findPolicyConflicts(files: ReturnType<typeof listMemoryFiles>): DreamOperation[] {
  const operations: DreamOperation[] = [];
  for (const file of files) {
    const text = readFileSync(file.absolutePath, "utf8").toLowerCase();
    if (text.includes("safari") && text.includes("user experience")) {
      operations.push({
        type: "conflict",
        targetFile: "decisions/README.md",
        reason: `${file.path} mentions both Safari-specific behavior and broader user-experience-first behavior. Review whether one supersedes the other.`,
        content: `\n<!-- Dream conflict review -->\n- ${file.path}: Safari-specific behavior may conflict with user-experience-first behavior.\n`,
        relatedFiles: [file.path]
      });
    }
  }
